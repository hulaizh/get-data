...
def get_full_path(service, folder):
    ...
    if not 'parents' in folder:
        return folder['name']
    files = service.files().get(
        fileId=folder['parents'][0], fields='id, name, parents').execute()
    path = files['name'] + ' > ' + folder['name']
    while 'parents' in files:
        files = service.files().get(
            fileId=files['parents'][0], fields='id, name, parents').execute()
...
https: // drive.google.com/file/d/13laqQARXRA_BHGgXXz9tp4LA04BpNvaE/view?usp = sharing

https: // drive.google.com/drive/folders/15m9KTGiLT0kfN7mFBhtuxCtdSCDZz3qq?usp = sharing
